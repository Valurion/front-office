...
class Configuration {
    ...
    /** Requête (utile pour déterminer le fichier de config à embarquer */
    private static $requete;

    /*
     * Garni la variable requete
     */

    public static function setRequete($requete) {
    ...
    public static function get($nom, $valeurParDefaut = null) {
        if (isset(self::getParametres()[$nom])) {
            ...
            throw new ConfigurationException("Key ".$nom." is not defined in configuration file");
        }
        return $valeur;
    }
    ...
    private static function getParametres() {
        if (self::$parametres == null) {
            $cheminFichier = "Config/" . (self::$requete->existeParametre("SERVER_NAME") ? self::$requete->getParametre("SERVER_NAME") : "") . ".ini";
            if (!file_exists($cheminFichier)) {
                $cheminFichier = "Config/local.ini";
                if (!file_exists($cheminFichier)) {
