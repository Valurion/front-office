...
function getDomainNameFromURIHint() {
    ...
    foreach ($IDProviders as $key => $value) {
        if (
                preg_match('/^http.+' . $clientDomainName . '/', $key) || preg_match('/^urn:.+' . $clientDomainName . '$/', $key)) {
            return $key;
        }
    }
...
function determineLanguage() {
    ...
    if (isset($_GET['lang'])) {
        ...
        if (
                $localeComponents !== false && isset($langStrings[$localeComponents[0]])
        ) {
...
function getIdPPathInfoHint() {
    ...
    foreach ($IDProviders as $key => $value) {
        ...
        if (
                isset($value['SSO']) && !empty($value['SSO']) && $value['Type'] != 'wayf' && isPartOfPathInfo(getHostNameFromURI($key))
        ) {
            return $key;
        }
    }
    ...
    foreach ($IDProviders as $key => $value) {
        // Only check actual IdPs
        if (
                isset($value['SSO']) && !empty($value['SSO']) && $value['Type'] != 'wayf' && isPartOfPathInfo(getDomainNameFromURI($key))
        ) {
            return $key;
        }
    }
...
function isPartOfPathInfo($needle) {
    if (
            isset($_SERVER['PATH_INFO']) && !empty($_SERVER['PATH_INFO']) && preg_match('|/' . $needle . '|', $_SERVER['PATH_INFO'])) {
...
function convertToShibDSStructure($IDProviders) {
    ...
    foreach ($IDProviders as $key => $value) {
        ...
        if (
                !isset($value['Type']) || $value['Type'] == 'category' || $value['Type'] == 'wayf'
        ) {
            continue;
        }
        ...
        foreach ($value as $lang => $name) {
            if (
                    $lang == 'Name' || $lang == 'SSO' || $lang == 'Realm' || $lang == 'Type' || $lang == 'IP'
            ) {
                continue;
            }
